...
export const PROCEDURE_LABELS: Record<string, string> = {
  eat_kotировки: 'ЕАТ «Берёзка» — закупка малого объёма',
...
export const PROCEDURE_DESCRIPTIONS: Record<string, string> = {
  eat_kotировки: 'Закупка у единственного поставщика через ЕАТ «Берёзка» (п.4 ч.1 ст.93 44-ФЗ). До 600 тыс. руб. Закупка малого объёма 2–24 ч, последние 10 мин — вслепую. Реестр контрактов в ЕАТ без публикации в ЕИС.',
...
export const BIDDING_STEPS: Record<string, Array<{status: string; name: string; description: string; order: number; responsible: string; requiredDocuments: string[]}>> = {
  ...
  eat_kotировки: [
    {
      status: 'placement',
      name: 'Закупка малого объёма на ЕАТ «Берёзка»',
      ...
      order: 5,
      responsible: 'Специалист МТО (Швецов К.Е.)',
      requiredDocuments: [
    ...
    },
    {
      ...
      order: 6,
      responsible: 'Площадка ЕАТ «Берёзка» / Швецов К.Е.',
      requiredDocuments: [
  ...
  single: [
    {
      ...
      order: 5,
      responsible: 'Специалист МТО (Швецов К.Е.) / Болдина А.В.',
      requiredDocuments: [
...
export const WORKFLOW_STEPS_BASE = [
  ...
  },
  {
    ...
    order: 2,
    responsible: 'Зам. руководителя (Толоконников Ю.В.)',
  ...
  },
  {
    ...
    order: 3,
    responsible: 'ФЭО / Финансово-экономический отдел (Пикинер О.В.)',
...
export const WORKFLOW_STEPS_POST = [
  {
    ...
    order: 8,
    responsible: 'Начальник МТО (Черемных М.Ю.)',
  ...
  },
  {
    status: 'payment',
    name: 'Оплата',
    description: 'Получение документов на оплату (счёт, акт, накладная). Передача пакета в специалист ФЭОию. Оформление платёжного поручения.',
    order: 13,
    responsible: 'Финансово-экономический отдел (Пикинер О.В.)',
...
function makeSteps(procId: string, completedUpTo: number, activeStep: number, procedure = 'eat_kotировки') {
  ...
  return template.map((s, i) => ({
    ...
    completedByName: s.order <= completedUpTo ? 'Швецов К.Е.' : undefined,
  }));
}
...
export const MOCK_PROCUREMENTS: Procurement[] = [
  {
    ...
    initiatorId: 'u_mit', initiatorName: 'Митусов С.А.',
    responsibleId: 'u_shv', responsibleName: 'Швецов К.Е.',
    plannedSum: 162000, contractSum: 156400, paidSum: 0,
    budgetCode: '051', kbk: '321 0412 54 4 01 90020 244', kosgu: '344',
  ...
  },
  {
    ...
    initiatorId: 'u_mit', initiatorName: 'Митусов С.А.',
    responsibleId: 'u_bol', responsibleName: 'Болдина А.В.',
    plannedSum: 4850000, contractSum: 4720000, paidSum: 0,
    budgetCode: '051', kbk: '321 0412 54 4 01 90020 242', kosgu: '310',
    financingSource: 'Федеральный бюджет',
  ...
  },
  {
    ...
    description: 'СЗ от Отдел общего обеспечения от 14.05.2026. Уборка 1250 кв.м. ежедневно плюс генеральные уборки.',
    status: 'sz_approval',
    riskLevel: 'low',
    departmentId: 'd1', departmentName: 'Отдел МТО',
    initiatorId: 'u_dav', initiatorName: 'Давыдова Ф.А.',
    responsibleId: 'u_shv', responsibleName: 'Швецов К.Е.',
    plannedSum: 375000,
    budgetCode: '051', kbk: '321 0412 54 4 01 90020 243', kosgu: '226',
    financingSource: 'Федеральный бюджет',
    ...
    documentIds: ['doc6'],
    tags: ['услуги', 'уборка', 'Отдел общего обеспечения'],
    priority: 'normal', isOverdue: false,
    szInitiatorDept: 'Отдел общего обеспечения', szDate: '2026-05-14',
    nmck: 375000,
  ...
  },
  {
    ...
    riskLevel: 'low',
    departmentId: 'd1', departmentName: 'Отдел МТО',
    initiatorId: 'u_shv', initiatorName: 'Швецов К.Е.',
    responsibleId: 'u_shv', responsibleName: 'Швецов К.Е.',
    plannedSum: 94500, contractSum: 91800, paidSum: 0,
    budgetCode: '051', kbk: '321 0412 54 4 01 90020 244', kosgu: '340',
    financingSource: 'Федеральный бюджет',
    ...
    szInitiatorDept: 'Отдел общего обеспечения', szDate: '2026-01-05',
  ...
  },
  {
    ...
    title: 'Разработка и внедрение интеграционный модульа для интеграции с ЕИС',
    description: 'СЗ от Отдела ИТ от 18.04.2026. Закупка по 44-ФЗ, электронный аукцион на ЕИС.',
    status: 'preparation',
    riskLevel: 'high',
    departmentId: 'd4', departmentName: 'ИТ-отдел',
    initiatorId: 'u_mit', initiatorName: 'Митусов С.А.',
    responsibleId: 'u_mit', responsibleName: 'Швецов К.Е.',
    plannedSum: 1850000,
    budgetCode: '051', kbk: '321 0412 54 4 01 90020 243', kosgu: '226',
    financingSource: 'Федеральный бюджет',
    procurementType: 'services',
    items: [
      { id:'i12', name:'Разработка интеграционный модульа', unit:'усл.', quantity:1, unitPrice:1400000, totalPrice:1400000 },
  ...
  },
  {
    ...
    departmentId: 'd1', departmentName: 'Отдел МТО',
    initiatorId: 'u_dav', initiatorName: 'Давыдова Ф.А.',
    responsibleId: 'u_dav', responsibleName: 'Болдина А.В.',
    ...
    priority: 'normal', isOverdue: false,
    szInitiatorDept: 'Отдел общего обеспечения', szDate: '2025-10-01',
  ...
  },
  {
    ...
    description: 'СЗ от Отдел общего обеспечения от 18.05.2026. Малый объём (ЕАТ Берёзка).',
    status: 'draft',
    riskLevel: 'low',
    departmentId: 'd1', departmentName: 'Отдел МТО',
    initiatorId: 'u_dav', initiatorName: 'Давыдова Ф.А.',
    ...
    plannedStartDate: '2026-07-01', plannedEndDate: '2027-06-30',
    documentIds: [],
    tags: ['страхование', 'транспорт', 'Отдел общего обеспечения'],
    priority: 'normal', isOverdue: false,
    szInitiatorDept: 'Отдел общего обеспечения', szDate: '2026-05-18',
    nmck: 112000,
  ...
  },
  {
    ...
    description: 'СЗ от Отдел общего обеспечения от 01.04.2026. Малый объём (ЕАТ Берёзка) в ЕАТ «Берёзка».',
    status: 'bidding',
    riskLevel: 'medium',
    departmentId: 'd1', departmentName: 'Отдел МТО',
    initiatorId: 'u_dav', initiatorName: 'Давыдова Ф.А.',
    ...
    documentIds: ['doc10'],
    tags: ['мебель', 'Отдел общего обеспечения', 'ЕАТ Берёзка'],
    priority: 'normal', isOverdue: false,
    szInitiatorDept: 'Отдел общего обеспечения', szDate: '2026-04-01',
    nmck: 445000,
  ...
  },
  {
    ...
    budgetCode: '051', kbk: '321 0412 54 4 01 90020 242', kosgu: '226',
    financingSource: 'Федеральный бюджет',
  ...
  },
  {
    ...
    description: 'СЗ от Отдел общего обеспечения от 26.04.2026. Плановое ТО. Малый объём (ЕАТ Берёзка) ЕАТ «Берёзка».',
    status: 'placement',
    riskLevel: 'medium',
    departmentId: 'd1', departmentName: 'Отдел МТО',
    initiatorId: 'u_dav', initiatorName: 'Давыдова Ф.А.',
    responsibleId: 'u_shv', responsibleName: 'Швецов К.Е.',
    plannedSum: 198000,
    budgetCode: '051', kbk: '321 0412 54 4 01 90020 243', kosgu: '225',
    financingSource: 'Федеральный бюджет',
    procurementType: 'services',
    items: [
    ...
    documentIds: [],
    tags: ['Отдел общего обеспечения', 'лифты', 'техобслуживание'],
    priority: 'normal', isOverdue: false,
    szInitiatorDept: 'Отдел общего обеспечения', szDate: '2026-04-26',
    nmck: 198000,
